...
In our regression model above, the total salary that each employee, i, received is represented as the predictor variable (X), and the response variable (Y) was the total benefits that individual employee received. $\beta_0$ is the intercept, $\beta_1$ is the slope, and $\epsilon_i$ is the residual error.
...
We also looked into the assumptions of the linear regression model on whether or not the relationship we have between our dependent and independent variables is linear and whether or not the residuals have constant variance, are independent, and are normally distributed.
